...
def slice_window(data: np.ndarray, sequence_length: int, batch_size: int) -> np.ndarray:
    ...
    Returns:
        结果
    """
    datas = timeseries_dataset_from_array(data, None, sequence_length=sequence_length, batch_size=batch_size, shuffle=False)
    result = tf.concat(list(datas), axis=0).numpy()
...
def get_turb_test_list(data: DataFrame, turb_id: int, _settings: dict) -> (tf.raw_ops.PrefetchDataset, Scaler):
    """
    获取一个风机的数据\n
    :param data: 原始数据
    :param turb_id: 风机id
